...
### Follow-up resolved — entry0178

The user authorized reversible repair. The previous Boltz runtime reproduced the same filesystem stall; preserving the shared directory and creating an empty replacement restored operation. Twelve repeated Boltz outputs were byte-identical to the earlier reference, two64-case PSICHIC checks passed, and the installed public prediction succeeded. Version0.2.2/build45/MCP23 adds a bounded startup health check. See [0178](0178-repair-mps-scratch-stall.md) for verified publication and remaining cross-Mac/long-term limits. The additional local checks described above are now resolved; original failed outputs remain intact.
