...
class Tests(unittest.TestCase):
 def test_retained_adapter_and_configuration(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   config=root/'request.json';original=dict(root=str(root),nested=[str(root/'rfd3'),str(root/'scripts/tool.py')],output=str(root/'projects/output'))
   ...
   self.assertEqual(bound['root'],str(view));self.assertEqual(bound['nested'][0],str(view/'rfd3'))
