...
def confidence_score(confidence, name, confidence_path, required=False):
    raw = confidence.get(name)
    if raw is None:
        study.require(not required, f"Missing required {name}: {confidence_path}")
        return None
    value = float(raw)
...
def evaluate(path, binder_sequence, target_sequence):
    ...
    values = {
        "iptm": confidence_score(confidence, "iptm", confidence_path, required=True),
        "complex_plddt": confidence_score(confidence, "complex_plddt", confidence_path),
    }
    ipsae = confidence_score(confidence, "ipsae_min", confidence_path)
...
def native_structures(directory, predictor, normalized):
    # Cycle-wave/resident schedules expose the selected native structure through
    # pred_min as a symlink. Validate the complete native prediction leaf rather
    # than incorrectly looking for batch output below the per-run cycle folder.
    if normalized.is_symlink():
        resolved = normalized.resolve(strict=True)
        candidates = sorted(
            path for path in resolved.parent.iterdir()
            if path.is_file() and path.suffix.lower() in (".cif", ".pdb")
        )
        study.require(resolved in candidates, f"Normalized structure does not resolve to its native leaf: {normalized}")
        return candidates
    if predictor == "boltz":
...
def check_logs(root, predictor):
    ...
    warnings, evidence = [], []
    resident_ready = root / "_cycle_wave/resident_ready.json"
    resident_mps = False
    if resident_ready.is_file():
        ready = study.read(resident_ready)
        resident_mps = ready.get("device") == "mps"
        if resident_mps:
            study.require(ready.get("fallback") == 0, f"Resident predictor reported fallback: {resident_ready}")
            study.require(int(ready.get("mps_driver_allocated_bytes", 0)) > 0,
                          f"Resident predictor lacks MPS allocation evidence: {resident_ready}")
            evidence.append({"path": str(resident_ready.relative_to(root)),
                             "record": {key: ready.get(key) for key in
                                        ("device", "fallback", "mps_driver_allocated_bytes", "model_load_count")}})
    ...
    for path in logs:
        ...
        study.require(markers or resident_mps, f"Missing explicit Apple GPU execution evidence: {path}")
        if markers:
...
def completion_receipts(root, run, cycles, state):
    """Validate the scheduler-specific success receipts for one finished job."""
    study.require(state.get("status") == "completed", "Broker did not record a completed job")
    run_receipt = run / "run_exit_code.txt"
    if run_receipt.is_file():
        study.require(run_receipt.read_text().strip() == "0", f"Nonzero run exit receipt: {run_receipt}")
        return True
    for cycle in cycles:
        receipts = sorted((root / "_cycle_wave" / f"cycle_{cycle:02d}").glob("batch_*/exit_code.txt"))
        study.require(receipts, f"Missing cycle-wave exit receipt for cycle {cycle:02d}")
        for receipt in receipts:
            study.require(receipt.read_text().strip() == "0", f"Nonzero cycle-wave exit receipt: {receipt}")
    return True
...
def audit(phase, arm_name):
    ...
    for local in range(1, count + 1):
        ...
        for metric in metrics:
            ...
            raw = native_structures(directory, arm["predictor"], structures[0])
            study.require(len(raw) == 1, f"Native prediction cardinality differs ({len(raw)}): {directory}")
            study.require(study.sha(raw[0]) == study.sha(structures[0]),
                          f"Normalized/native prediction bytes differ: {directory}")
        ...
        expected_cycles = list(range(campaign["optimization_cycles"] + 1))
        completed = ([row["cycle"] for row in this_run] == expected_cycles
                     and completion_receipts(root, run, expected_cycles, state))
        if completed:
    ...
    value = {
        ...
        "missing_optional_metrics": {
            ...
            for name in ("complex_plddt", "ipsae_min")
        },
