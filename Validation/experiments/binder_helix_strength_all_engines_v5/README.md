...
```bash
python3 Validation/experiments/binder_helix_strength_all_engines_v5/campaign.py \
  validate-target --target-name my_target --target-sequence-file /path/target.fasta \
  --target-msa /path/target.a3m
python3 Validation/experiments/binder_helix_strength_all_engines_v5/campaign.py stage
python3 Validation/experiments/binder_helix_strength_all_engines_v5/campaign.py \
  prepare --target-name my_target --target-sequence-file /path/target.fasta \
  --target-msa /path/target.a3m
python3 Validation/experiments/binder_helix_strength_all_engines_v5/campaign.py plan --pilot-only
python3 Validation/experiments/binder_helix_strength_all_engines_v5/campaign.py run --pilot-only
python3 Validation/experiments/binder_helix_strength_all_engines_v5/campaign.py run
```
...
leaves them null otherwise; iPTM remains required for every trajectory.
