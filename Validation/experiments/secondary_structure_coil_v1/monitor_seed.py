...
from seed_campaign import base, HERE
...
def worker():
    ...
    with (output / 'monitor.lock').open('a+') as lock:
        ...
        while True:
            ...
            if set(states) != set(base.CONFIG['arms']):
                raise RuntimeError('Submitted status arm set differs from declaration')
        ...
        subprocess.run([str(python), str(HERE / 'audit_seed.py'), '--root', str(base.OUTPUT),
        ...
        if not gate.get('passed'):
            ...
            return 1
        subprocess.run([str(python), str(HERE / 'summarize_seed.py'), '--root', str(base.OUTPUT)],
                       cwd=base.ROOT, env=environment, check=True)
        base.atomic_json(output / 'monitor_status.json',
