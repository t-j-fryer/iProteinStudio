...
class CampaignContracts(unittest.TestCase):
    def test_openfold_msa_adapter_is_frozen(self):
        self.assertIn("scripts/openfold_query_json.py", study.STAGED)
    ...
    def test_full_cohort_requires_every_pilot_arm(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary)
            audits = output / "audits/pilot"
            audits.mkdir(parents=True)
            for arm_name in study.CONFIG["arms"]:
                study.atomic(audits / f"{arm_name}.json", {"operational_passed": True})
            failed = next(iter(study.CONFIG["arms"]))
            study.atomic(audits / f"{failed}.json", {"operational_passed": False})
            with mock.patch.object(study, "OUTPUT", output):
                with self.assertRaisesRegex(RuntimeError, f"failed for {failed}"):
                    study.require_global_pilot_gate()
                study.atomic(audits / f"{failed}.json", {"operational_passed": True})
                study.require_global_pilot_gate()
    ...
    def test_cycle_wave_completion_requires_zero_receipt_for_every_cycle(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            run = root / "run_001"
            run.mkdir()
            for cycle in range(6):
                receipt = root / "_cycle_wave" / f"cycle_{cycle:02d}" / "batch_01/exit_code.txt"
                receipt.parent.mkdir(parents=True)
                receipt.write_text("0\n")
            self.assertTrue(audit.completion_receipts(root, run, range(6), {"status": "completed"}))
            (root / "_cycle_wave/cycle_05/batch_01/exit_code.txt").write_text("1\n")
            with self.assertRaisesRegex(RuntimeError, "Nonzero cycle-wave"):
                audit.completion_receipts(root, run, range(6), {"status": "completed"})

    def test_engine_conditional_confidence_metrics_may_be_absent(self):
        confidence = {"iptm": 0.75}
        path = Path("confidence.json")
        self.assertEqual(audit.confidence_score(confidence, "iptm", path, required=True), 0.75)
        self.assertIsNone(audit.confidence_score(confidence, "complex_plddt", path))
        self.assertIsNone(audit.confidence_score(confidence, "ipsae_min", path))
        with self.assertRaisesRegex(RuntimeError, "Missing required iptm"):
            audit.confidence_score({}, "iptm", path, required=True)

    def test_resident_ready_receipt_is_explicit_mps_evidence(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            log = root / "run_001/cycle_00/intellifold/predict.log"
            log.parent.mkdir(parents=True)
            log.write_text("prediction completed\n")
            receipt = root / "_cycle_wave/resident_ready.json"
            receipt.parent.mkdir(parents=True)
            study.atomic(receipt, {"device": "mps", "fallback": 0,
                                   "mps_driver_allocated_bytes": 1024, "model_load_count": 1})
            warnings, evidence = audit.check_logs(root, "intellifold")
            self.assertEqual(warnings, [])
            self.assertEqual(evidence[0]["record"]["device"], "mps")
