...
SOURCE = ROOT / "Validation/output/binder_helix_strength_all_engines_v7/source"
RUNTIME = Path(os.environ.get("NANOHUNTER_ROOT", str(Path.home() / ".iproteinstudio"))).resolve()
OUTPUT = ROOT / "Validation/output/binder_helix_strength_all_engines_v7"
PROJECT = "validation_binder_helix_strength_all_engines_v7"
...
STAGED = (
    ...
    "scripts/openfold_query_json.py",
...
def require_global_pilot_gate():
    """Refuse every expanded arm until the complete pilot matrix has passed."""
    for arm_name in CONFIG["arms"]:
        path = OUTPUT / "audits/pilot" / f"{arm_name}.json"
        require(path.is_file(), f"Global pilot gate is incomplete: missing {arm_name}")
        outcome = read(path)
        require(outcome.get("operational_passed") is True,
                f"Global pilot gate failed for {arm_name}")
...
def run(only_arm=None, pilot_only=False):
    ...
    with (OUTPUT / "controller.lock").open("a+") as lock:
        ...
        if not pilot_only:
            work += [("remaining", arm_name) for arm_name in arm_names]
        remaining_gate_checked = False
        for phase, arm_name in work:
            if phase == "remaining" and not remaining_gate_checked:
                require_global_pilot_gate()
                remaining_gate_checked = True
