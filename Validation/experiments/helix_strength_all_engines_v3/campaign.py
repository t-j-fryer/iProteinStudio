...
SOURCE = ROOT / "Validation/output/helix_strength_all_engines_v3/source"
RUNTIME = Path(os.environ.get("NANOHUNTER_ROOT", str(Path.home() / ".iproteinstudio"))).resolve()
OUTPUT = ROOT / "Validation/output/helix_strength_all_engines_v3"
PROJECT = "validation_helix_strength_all_engines_v3"
...
STAGED = ("nanohunter_run.sh", "scripts/secondary_structure_control.py",
          ...
          "mcp/schemas/iterative-design-v1.json", "mcp/iprotein_mcp/catalog.py",
          "mcp/MCP_VERSION", "mcp/iprotein_mcp/__init__.py", "mcp/README.md",
          "scripts/validate_prediction_geometry.py", "scripts/boltz_mps.py",
          "scripts/intellifold_predict.py", "scripts/resident_predictor.py")
...
def stage_preview():
    return {"files": {name: {"before_sha256": sha(RUNTIME / name) if (RUNTIME / name).is_file() else None,
                             "after_sha256": sha(SOURCE / name if (SOURCE / name).exists() else WORKING_SOURCE / name)} for name in STAGED},
...
def stage():
    # Freeze the exact reviewed source independently of concurrent app development.
    for name in STAGED:
    ...
    with (RUNTIME / "agent/execution.lock").open("a+") as lock:
        ...
        engine_paths = [p for folder in ("models", "receipts", "locks")
                        for p in (RUNTIME / folder).rglob("*") if p.is_file()]
        engine_paths += [RUNTIME / "src/LigandMPNN/model_params/solublempnn_v_48_020.pt"]
        receipt["engine_files"] = {str(p.relative_to(RUNTIME)): fingerprint(p) for p in engine_paths}
        ...
        receipt["engine_code"] = {str(p.relative_to(RUNTIME)): fingerprint(p) for p in code_paths
                                  if str(p.relative_to(RUNTIME)) not in STAGED}
...
def arguments_for(arm, phase):
    ...
    args = ["--workflow", "protein", "--predictor", arm["predictor"], "--sequence-designer", c["sequence_designer"],
            ...
            "--binder-percent-x", "50", "--negative-helix-constant", str(arm["strength"]),
    ...
    with (OUTPUT / "controller.lock").open("a+") as lock:
        ...
        for arm_name in arm_names:
            for phase in (["pilot"] if pilot_only else ["pilot", "remaining"]):
                if phase == "pilot" and arm_name in CONFIG.get("reused_pilots", {}):
                    result = subprocess.run([str(RUNTIME / "venvs/NanoHunter_protenix/bin/python"),
                        str(HERE / "audit.py"), "--phase", phase, "--arm", arm_name], capture_output=True, text=True,
        ...
        if not pilot_only and not only_arm:
            ...
            import finish
            atomic(OUTPUT / "analysis_status.json", finish.finish())
