...
def main():
 ...
 sys.path.insert(0,req.get('scripts',str(root/'scripts')))
 import numpy as np
 import torch
 ...
 if engine == 'boltz':
  ...
  return run_boltz(req,out,a.request,start)
 if req.get('diagnostic') in ('kernel','norm_kernel'):
  if req['diagnostic']=='norm_kernel':from norm_kernel import run
  else:from kernel_benchmark import run
 ...
 if engine.startswith('intellifold') or engine in ('protenix','constraint'):
  ...
  if engine.startswith('intellifold'):
   ...
   if req.get('variant')=='buckets128':config['engine_args']+=['--buckets','128,256,512,768,1024,1280,1536,2048,2560,3072,3584,4096,4608,5120']
   if req.get('variant')=='padding256':config['engine_args']+=['--buckets','256,512,768,1024,1280,1536,2048,2560,3072,3584,4096,4608,5120']
  ...
  sys.path.insert(0,str(Path(req.get('scripts',str(root/'scripts')))/'nise'))
  import nesso_worker
  session=nesso_worker.Engine(Path(req['managed_root']), ccd_cache=req.get('variant') != 'uncached', preprocessing_cache=req.get('variant')=='conformer_cache')
  if req.get('variant')=='preprocess':
   from nesso_preprocess import install
   preprocess_counts=install(session)
 ...
 telemetry={}
 if req.get('variant')=='metal':
  from fusion import install
  model=session.model if engine=='nesso' else session.runner.model
  telemetry=install(model,engine,torch)
 if req.get('detailed_profile'):
  from module_profile import install as instrument
  model=session.model if engine=='nesso' else session.runner.model
  telemetry=instrument(model,torch)
 ...
 for i,case in enumerate(req['cases']):
  ...
  if engine=='nesso':
   details=session.score(case['sequence'],case.get('smiles','CC(=O)O'),unit/'prediction',req['seed'])
  else:
   import yaml
  ...
  if engine=='nesso' and req.get('variant')=='preprocess':details['preprocess']=dict(preprocess_counts)
 ...
 atomic(out/'telemetry.json',telemetry)
