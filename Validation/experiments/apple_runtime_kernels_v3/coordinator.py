...
def main():
 ...
 for b in cfg['blocks']:
  ...
  if complete:
   ...
   request=dict(**b,root=cfg['root'],scripts=cfg['scripts'],managed_root=cfg['managed_root'],output=str(attempt))
   ...
   for line in fallback:
    if b['engine']!='boltz' or 'linalg_svd' not in line:raise RuntimeError('Forbidden CPU fallback: '+line)
   if code:
