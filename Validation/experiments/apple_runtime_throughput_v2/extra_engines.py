...
def run_extra(req,out,request,start):
 ...
 for i,case in enumerate(req['cases']):
  ...
  if engine=='rfd3':
   ...
   query=unit/'query.json';atomic(query,dict(seeds=[req['seed']],queries={case['name']:dict(use_msas=False,use_paired_msas=False,use_main_msas=False,chains=[dict(molecule_type='protein',chain_ids=['A'],sequence=case['sequence'])])}))
   ...
   details=dict(transfer);details['float32_matmul_precision']=torch.get_float32_matmul_precision();details['measurement_scope']='includes a new model load for each input'
  elif engine=='antifold':
