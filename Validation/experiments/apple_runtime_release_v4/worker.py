...
def main():
 ...
 if engine.startswith('intellifold') or engine in ('protenix','constraint'):
  ...
  config=dict(root=str(root),seed=req['seed'],samples=1,use_msa=bool(req.get('use_msa',False)))
  msa_features=[]
  if engine.startswith('intellifold'):
   ...
   def record_shapes(module,args):
    ...
    shapes={k:list(v.shape) for k,v in features.items() if 'msa' in k and hasattr(v,'shape')}
    msa_features.append(shapes)
    if req.get('use_msa'):assert features['msa'].shape[-3]>1,shapes
   ...
   assert cfg.dtype=='fp32' and not cfg.enable_efficient_fusion and not cfg.enable_tf32 and not cfg.use_template and bool(cfg.use_msa)==bool(req.get("use_msa",False))
   ...
   original_predict=session.runner.predict
   def record_msa(data):
    features=data['input_feature_dict'];shapes={k:list(v.shape) for k,v in features.items() if 'msa' in k and hasattr(v,'shape')}
    msa_features.append(shapes)
    if req.get('use_msa'):assert features['msa'].shape[0]>1,shapes
    return original_predict(data)
   session.runner.predict=record_msa
 ...
 for i,case in enumerate(req['cases']):
  ...
  if engine=='nesso':
   ...
   msa=case.get('msa','empty')
   if req.get('use_msa'):
    assert msa!='empty' and sha(msa)==case['msa_sha256']
    assert Path(msa).read_text().splitlines()[1]==case['sequence']
   (source/(case['name']+'.yaml')).write_text(yaml.safe_dump(dict(version=1,sequences=[dict(protein=dict(id='A',sequence=case['sequence'],msa=msa))]),sort_keys=False))
  ...
  if engine.startswith('intellifold') or engine in ('protenix','constraint'):
   details['msa_feature_shapes']=msa_features[-1:]
   details['msa_sha256']=case.get('msa_sha256')
