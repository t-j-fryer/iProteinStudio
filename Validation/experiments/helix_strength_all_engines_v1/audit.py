...
def audit(phase, arm_name):
    ...
    for name in ('nanohunter_run.sh', 'scripts/secondary_structure_control.py', 'scripts/initialization_assessment.py'):
    ...
    for local in range(1, count + 1):
        ...
        for metric in metrics:
            ...
            issues = geometry.validate(structures[0])
            study.require(not issues, f'Accepted prediction has invalid geometry: {issues}')
            row = dict(arm=arm_name, phase=phase, trajectory=global_index, cycle=cycle, sequence=seq,
                       structure=str(structures[0].relative_to(study.OUTPUT)), **evaluate(structures[0], seq))
