...
SOURCE = ROOT / "Validation/output/binder_helix_strength_all_engines_v1/source"
RUNTIME = Path(os.environ.get("NANOHUNTER_ROOT", str(Path.home() / ".iproteinstudio"))).resolve()
OUTPUT = ROOT / "Validation/output/binder_helix_strength_all_engines_v1"
PROJECT = "validation_binder_helix_strength_all_engines_v1"
...
def prepare(target_name=None, sequence_path=None, msa_path=None):
    ...
    template = OUTPUT / "inputs/input_template.yaml"
    msa_yaml = json.dumps(str(frozen_msa))
    atomic_text(template,
                "sequences:\n"
    ...
    manifest = {
        ...
        "template": {"path": str(template.relative_to(OUTPUT)), "sha256": sha(template)},
...
def ensure_plan(phase, arm_name):
    ...
    template = OUTPUT / manifest["template"]["path"]
...
def main():
    ...
    parser.add_argument("action", choices=["validate-target", "stage-preview", "stage", "prepare", "run", "status"])
    ...
    if args.action == "validate-target":
        ...
        emit({"stage-preview": stage_preview, "stage": stage, "status": status}[args.action]())
