...
```bash
python3 Validation/experiments/binder_helix_strength_all_engines_v3/campaign.py \
  validate-target --target-name my_target --target-sequence-file /path/target.fasta \
  --target-msa /path/target.a3m
python3 Validation/experiments/binder_helix_strength_all_engines_v3/campaign.py stage
python3 Validation/experiments/binder_helix_strength_all_engines_v3/campaign.py \
  prepare --target-name my_target --target-sequence-file /path/target.fasta \
  --target-msa /path/target.a3m
python3 Validation/experiments/binder_helix_strength_all_engines_v3/campaign.py plan --pilot-only
python3 Validation/experiments/binder_helix_strength_all_engines_v3/campaign.py run --pilot-only
python3 Validation/experiments/binder_helix_strength_all_engines_v3/campaign.py run
```
...
files. V3 follows the normalized output link to the native prediction leaf,
checks that leaf's cardinality and byte identity, and otherwise preserves the
same declared design. It does not reuse V1 or V2 inference output.
